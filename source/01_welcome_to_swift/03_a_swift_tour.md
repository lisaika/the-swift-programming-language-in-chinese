...
编程语言教程的第一个程序通常是在屏幕上打印“Hello, world”。在 Swift 中，可以用一行代码实现：

```swift
print("Hello, world!")
```

如果你写过 C 或者 Objective-C 代码，那你应该很熟悉这种形式——在 Swift 中，这行代码就是一个完整的程序。你无需为了输入输出或者字符串处理等功能导入一个单独的库。全局作用域中的代码会自动用作程序的入口点，所以你也无需 `main()` 函数。同样你也无需在每个语句结尾写上分号。

本教程通过一系列编程示例让你初步掌握 Swift。如果遇到不理解的地方也不用担心——我们会在后面的章节展开介绍本章所涉及的内容。

> 注意
> 
> 为了获取最佳体验，建议将本章作为 Playground 文件在 Xcode 中打开。 通过 Playground，你可以编辑代码并立即查看输出结果。
> 
> [下载 Playground](https://docs.swift.org/swift-book/GuidedTour/GuidedTour.playground.zip)

## 简单值 {#simple-values}

使用 `let` 来声明常量，使用 `var` 来声明变量。编译时，常量并不需要有明确的值，但是你只能为常量赋值一次。这说明你可以用一个常量来命名一个值，一次赋值即可在多处使用。
...
常量或者变量的类型必须与所赋值的类型相同。然而，你无需明确地声明类型，因为当你通过一个值声明变量和常量时，编译器会自动推断其类型。在上面的例子中，编译器推断出 `myVariable` 是一个整数类型，因为它的初始值是整数。

如果初始值未提供足够的信息（或者没有初始值），则需要在变量后面声明类型，并用冒号隔开。
...
值永远不会被隐式转换为其他类型。如需将值转换成其他类型，请显式转换。
...
有一种更简单的把值转换成字符串的方法：把值写到括号中，并在括号前加上一个反斜杠（\）。例如：
...
> 练习
> 
> 使用 `\()` 来把一个浮点计算转换成字符串，并加上某人的名字。

使用三个双引号（`"""`）来包含多行字符串内容，字符串中的内容（包括引号、空格、换行符等）都会保留下来。例如：
...
使用 `if` 和 `switch` 来进行条件操作，使用 `for-in`、`while` 和 `repeat-while` 来进行循环。可以省略条件和循环变量外面的括号，但是不能省略语句体的大括号。
...
你可以一起使用 `if` 和 `let` 来处理值缺失的情况。这些值可由可选值来代表。可选值要么包含一个具体的值，要么包含 `nil` ，表示值缺失。在类型后面加一个问号（`?`）来标记这个变量的值是可选的。
